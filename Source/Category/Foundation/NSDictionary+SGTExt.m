...
@interface _SGTXMLDictionaryParser : NSObject <NSXMLParserDelegate>
@end

@implementation _SGTXMLDictionaryParser {
...
@implementation NSDictionary (SGTExt)
...
+ (NSDictionary *)dictionaryWithXML:(id)xml {
    _SGTXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_SGTXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_SGTXMLDictionaryParser alloc] initWithData:xml];
    }
