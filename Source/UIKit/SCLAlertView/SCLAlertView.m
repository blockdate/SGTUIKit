...
- (SCLButton *)addButton:(NSString *)title command:(RACCommand *)command {
    SCLButton *btn = [self addButton:title];
...
- (void)buttonTapped:(SCLButton *)btn
{
    ...
    {
        APP_DebugLogBlue(@"Unknown action type for button");
    }
...
- (SCLAlertViewBuilder *(^) (NSString * title, RACCommand *command))addButtonWithCommand {
    if (!_addButtonWithCommand) {
        __weak typeof(self) weakSelf = self;
        _addButtonWithCommand = ^(NSString *title, RACCommand *command) {
            [weakSelf.alertView addButton:title command:command];
            return weakSelf;
        };
    }
    return _addButtonWithCommand;
}
